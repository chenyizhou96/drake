...
template <typename T>
void AdmmSolver<T>::CalcVelocityAndImpulses(
...
template <typename T>
T AdmmSolver<T>::CalcCostAndGradients(
...
template <typename T>
T AdmmSolver<T>::CalcLineSearchCostAndDerivatives(
...
template <typename T>
int AdmmSolver<T>::CalcLineSearchParameter(const State& state,
                                                          T* alpha) const {
  using std::abs;
  using std::max;
  using std::min;
...
template <typename T>
void AdmmSolver<T>::CallSupernodalSolver(
...
template <typename T>
void AdmmSolver<T>::CallDenseSolver(const State& state,
                                                   VectorX<T>* dv) {
  Timer local_timer;
  const int nv = data_.nv;
